...
- a delegated `39000` event accepted only because the bound group has no current durable members (the creation bootstrap) MUST NOT apply lockout or destructive metadata to that group: it MUST NOT carry a `["status", "tombstoned"]` tag, the `closed` flag, or any `["ban", "<mask>"]` tag; those are operator actions on an established channel, and permitting them at bootstrap would let any delegated party permanently tombstone an unclaimed channel (which has no retained operator to reactivate it) or pre-seed closed/ban state that the operator who later claims the channel silently inherits
